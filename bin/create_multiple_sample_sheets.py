...
EXT_MB_DRAGEN = '_DGNPPG.csv'
EXT_REG = '.csv'
# STEP 2 - ADD EXT_* VARIABLE HERE
EXTENSIONS = [EXT_10X, EXT_MLT, EXT_DLP, EXT_PAD, EXT_WGS, EXT_PPG, EXT_6NT, EXT_REG, EXT_MB_DNA, EXT_MB_PTN, EXT_MB_DRAGEN]
...
DF_IDX_DGN = EXTENSIONS.index(EXT_MB_DRAGEN)
...
def wgs(sample_data, header):
	...
	if not ped_peg_data.empty:
		...
		DATA_SHEETS[DF_IDX_DGN] = ped_peg_data.copy()       # We create a copy of PED_PEG and send it to DRAGEN
