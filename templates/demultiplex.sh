...
OUTPUT_DIR="!{STATS_DIR}/${RUN_BASENAME}"

# TODO - undo this
OUTPUT_DIR=$(pwd)/${RUN_BASENAME}
...
if /bin/grep -q "10X_Genomics" $SAMPLESHEET; then
  ...
  if /bin/grep -q "10X_Genomics_ATAC" $SAMPLESHEET; then
    JOB_CMD="/home/nabors/cellranger-atac-1.1.0/cellranger-atac mkfastq --input-dir ${RUNPATH} --sample-sheet ${ORIGINAL_SAMPLE_SHEET} --output-dir ${OUTPUT_DIR} --nopreflight --jobmode=lsf --mempercore=32 --disable-ui --maxjobs=200 --barcode-mismatches 1"
    echo "Running cellranger-atac mkfastq: ${JOB_CMD}"
  else
    JOB_CMD="/igo/work/bin/cellranger-4.0.0/cellranger mkfastq --input-dir $RUNPATH/ --sample-sheet $ORIGINAL_SAMPLE_SHEET --output-dir ${OUTPUT_DIR} --nopreflight --jobmode=local --localmem=216 --localcores=36  --barcode-mismatches 1"
    echo "Running cellranger-atac mkfastq: ${JOB_CMD}"
  fi
else
  export LD_LIBRARY_PATH=/opt/common/CentOS_6/gcc/gcc-4.9.2/lib64:$LD_LIBRARY_PATH
  JOB_CMD="/opt/common/CentOS_6/bcl2fastq/bcl2fastq2-v2.20.0.422/bin/bcl2fastq --minimum-trimmed-read-length 0 --mask-short-adapter-reads 0 --ignore-missing-bcl  --runfolder-dir  $RUNPATH/ --sample-sheet $ORIGINAL_SAMPLE_SHEET --output-dir ${OUTPUT_DIR} --ignore-missing-filter --ignore-missing-positions --ignore-missing-control --barcode-mismatches 1 --no-lane-splitting  --loading-threads 12 --processing-threads 24 2>&1 >> /home/igo/log/bcl2fastq.log"
  echo "Running bcl2fastq with 1 mismatch RUN=$RUN_TO_DEMUX RUNPATH=$RUNPATH OUTPUT=${OUTPUT_DIR} SAMPLESHEET=$ORIGINAL_SAMPLE_SHEET: ${JOB_CMD}"
fi
...
echo ${OUTPUT_DIR} > !{DEMUXED_RUN_DIRS}
